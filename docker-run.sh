...
chmod 777 $DOC_ROOT/runtime
chmod 777 $DOC_ROOT/web/upload
...
if [ "$MYSQL_HOST" != "" ] && [ "$MYSQL_DATABASE" != "" ] && [ $MYSQL_USER != "" ]; then
	...
	if [ $max_tries -eq 0 ]; then
		...
		if [ ${#tables[@]} -eq 0 ]; then
			...
			# Schema
			mysql --defaults-extra-file=$CREDS -h"${MYSQL_HOST}" ${MYSQL_DATABASE} < $DOC_ROOT/data/schema.mysql.sql
			# Base data
			mysql --defaults-extra-file=$CREDS -h"${MYSQL_HOST}" ${MYSQL_DATABASE} < $DOC_ROOT/data/base_data.mysql.sql
			echo " done."
		else
